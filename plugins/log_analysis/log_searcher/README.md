...
```
The file is registered as a `json_events` session artifact containing matched lines, line numbers, context, and match count. Use `artifacts` to get its ID, then pass it to `log_investigator` or `log_pattern_analyzer` for deeper analysis.
